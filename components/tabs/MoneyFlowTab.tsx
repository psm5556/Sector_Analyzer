...
const SUB_TABS: { id: SubTab; label: string; desc: string }[] = [
  { id: 'treemap', label: '🗺️ 트리맵', desc: '섹터/종목별 시총 크기, 색=수익률' },
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        {subTab === 'treemap' && (
          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3">
              트리맵 — 블록 크기: 시총, 색상: 누적 수익률
            </h3>
            <TreemapChart data={treemapData} />
            <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#1a7a4a' }} /> +10% 이상</div>
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#52b788' }} /> +3~10%</div>
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#b7e4c7' }} /> 0~3%</div>
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#ffc8c8' }} /> 0~-3%</div>
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#dc2f02' }} /> -3~-10%</div>
              <div className="flex items-center gap-1"><span className="inline-block w-3 h-3 rounded" style={{ background: '#9d0208' }} /> -10% 이하</div>
            </div>
          </div>
        )}
      ...
      <div className="text-xs text-gray-400 text-right">
        {results.length}개 종목 · {treemapData.children.length}개 섹터 분석
      </div>
    </div>
  );
}
